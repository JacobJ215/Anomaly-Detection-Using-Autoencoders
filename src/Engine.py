...
from ML_Pipeline.util import load_model, save_model
import pandas as pd
import subprocess
...
if val == 0:
    ...
    processed_df = preprocess_data(data, is_train=True)
    ml_model, columns = Train_Model.fit(processed_df)
    model_path = save_model(ml_model, columns)
    print("Model saved in: ", "../output/autoencoder-model")
elif val == 1:
    ...
    processed_df = preprocess_data(test_data, is_train=False)
